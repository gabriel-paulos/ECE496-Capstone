...
def extracted_audio_files():
    sample_dir = "test/test_sample_data/test_transcript_lexeme_identification_data"
...
def test_transcript_lexeme_identification(extracted_audio_file, golden_transcript_file):
    ...
    umm_lexemes = ["UM", "UMM", "UMMM", "UMMMMM", "UHM", "UHHHHM", "UHHM", "EUHM", "AM", "AMM", "AHM"]

    modified_transcript_words = golden_transcript.split("|")

    for i in range(len(modified_transcript_words)):
        if modified_transcript_words[i] in umm_lexemes:
            modified_transcript_words[i] = random.choice(umm_lexemes)
    ...
    for i in range(len(golden_path)):
        assert modified_path[i].label in umm_lexemes
